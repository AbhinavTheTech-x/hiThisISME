...
describe('solana-farm', () => {
  ...
  // data account
  const mangoFarmKeypair = getMangoFarmKeyPair();
  const dev = getDevKeyPair();

  const { alice, bob, referral } = getAccounts();

  before('#setup', async () => {
    let mangoFarmVault = getPDA(program.programId, SOLANA_FARM_VAULT);
    await requestAirdropMulti(
      programProvider,
      [alice.publicKey, bob.publicKey, referral.publicKey, dev.publicKey, mangoFarmVault],
      LAMPORTS
    );
  });
  ...
  describe('#initialize', () => {
    ...
    it('try initializing with invalid owner', async () => {
      await expect(initialize(program, [mangoFarmKeypair, alice], alice.publicKey)).to.be.rejected;
    });
    ...
    it('initialize tx should confirm', async () => {
      await expect(initialize(program, [mangoFarmKeypair], program.provider.publicKey as any)).to
        .be.fulfilled;
    });

    it('check state', async () => {
      const mangoFarmState = await getMangoFarmState(program, mangoFarmKeypair.publicKey);
      expect(mangoFarmState.initialized).to.be.true;
      expect(mangoFarmState.marketEggs.eq(new anchor.BN('108000000000'))).to.be.true;
    });
  ...
  describe('#buyEggs', () => {
    ...
    it('check solana farm state', async () => {
      let mangoFarm = await getMangoFarmState(program, mangoFarmKeypair.publicKey);
      expect(mangoFarm.marketEggs.toNumber()).to.be.equal(111312000000);
    });

    it('check sol rewards', async () => {
      let rewards = await getEarnedSFarm(program, mangoFarmKeypair.publicKey, alice.publicKey);
      expect(rewards.toNumber()).to.be.gt(0);
    });

    it('bob should buy with 0.5 sol without referral', async () => {
      await expect(
        buyEggs(program, new anchor.BN(5e8), [bob], mangoFarmKeypair.publicKey, dev.publicKey)
      ).to.fulfilled;
    });
  });
  ...
  describe('#hatchEggs', () => {
    it('alice should hatch eggs', async () => {
      await expect(hatchEggs(program, [alice], mangoFarmKeypair.publicKey, referral.publicKey)).to
        .be.fulfilled;
    });

    it('bob should hatch eggs with same referral revert', async () => {
      await expect(hatchEggs(program, [bob], mangoFarmKeypair.publicKey, bob.publicKey)).to.be
        .rejected;
    });

    it('bob should hatch eggs', async () => {
      await expect(hatchEggs(program, [bob], mangoFarmKeypair.publicKey)).to.be.fulfilled;
    });
  });
  ...
  describe('#sellEggs', () => {
    ...
    it('alice sell eggs but passing different dev pubkey', async () => {
      const mangoFarmVault = getPDA(program.programId, SOLANA_FARM_VAULT);
      await expect(sellEggs(program, [alice], mangoFarmVault, mangoFarmKeypair.publicKey)).to.be
        .rejected;
    });

    it('alice sell eggs', async () => {
      devBalancebefore = await getLamports(programProvider, dev.publicKey);
      await expect(sellEggs(program, [alice], dev.publicKey, mangoFarmKeypair.publicKey)).to.be
        .fulfilled;
    });

    it('referral sell eggs', async () => {
      await expect(sellEggs(program, [referral], dev.publicKey, mangoFarmKeypair.publicKey)).to.be
        .fulfilled;
    });
